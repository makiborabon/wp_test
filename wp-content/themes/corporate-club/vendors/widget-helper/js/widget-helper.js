var corporate_club_upload_frame_obj;
...
( function( $ ) {
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var corporateClubImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'corporate-club-state-insert',
			...
			corporate_club_upload_frame_obj = wp.media.frames.corporate_club_upload_frame_obj = wp.media({
				button: {
					text: $( this ).data( 'uploader_button_text' )
				},
				state : 'corporate-club-state-insert',
				states : [
					new corporateClubImageObject()
				],
				multiple: false
			});

			corporate_club_upload_frame_obj.on( 'select', function() {

				var state = corporate_club_upload_frame_obj.state('corporate-club-state-insert');
			...
			corporate_club_upload_frame_obj.open();
		});
