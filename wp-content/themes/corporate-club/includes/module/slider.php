...
if ( ! function_exists( 'corporate_club_add_featured_slider' ) ) :
	 ...
	 * @since 1.0.0
	 */
	function corporate_club_add_featured_slider() {

		$flag_apply_slider = apply_filters( 'corporate_club_filter_slider_status', true );
		...
		$slider_details = apply_filters( 'corporate_club_filter_slider_details', $slider_details );

		if ( empty( $slider_details ) ) {
			return;
		}

		corporate_club_render_featured_slider( $slider_details );

	}

endif;

add_action( 'corporate_club_action_before_content', 'corporate_club_add_featured_slider', 5 );

if ( ! function_exists( 'corporate_club_check_slider_status' ) ) :
	 ...
	 */
	function corporate_club_check_slider_status( $input ) {

		$input = false;

		// Slider status.
		$featured_slider_status = corporate_club_get_option( 'featured_slider_status' );
...
add_filter( 'corporate_club_filter_slider_status', 'corporate_club_check_slider_status' );

if ( ! function_exists( 'corporate_club_get_slider_details' ) ) :
	 ...
	 */
	function corporate_club_get_slider_details( $input ) {

		$featured_slider_type           = corporate_club_get_option( 'featured_slider_type' );
		$featured_slider_number         = corporate_club_get_option( 'featured_slider_number' );
		$featured_slider_read_more_text = corporate_club_get_option( 'featured_slider_read_more_text' );
		...
		switch ( $featured_slider_type ) {
			...
			case 'featured-page':
				...
				for ( $i = 1; $i <= $featured_slider_number ; $i++ ) {
					$id = corporate_club_get_option( 'featured_slider_page_' . $i );
					if ( ! empty( $id ) ) {
						$ids[] = absint( $id );
					}
				}
				...
				if ( ! empty( $all_posts ) ) {
					...
					foreach ( $all_posts as $key => $post ) {
						...
						if ( has_post_thumbnail( $post->ID ) ) {
							...
							$slides[ $cnt ]['excerpt'] = corporate_club_get_the_excerpt( apply_filters( 'corporate_club_filter_slider_caption_length', 30 ), $post );
...
add_filter( 'corporate_club_filter_slider_details', 'corporate_club_get_slider_details' );

if ( ! function_exists( 'corporate_club_render_featured_slider' ) ) :
	 ...
	 */
	function corporate_club_render_featured_slider( $slider_details = array() ) {

		if ( empty( $slider_details ) ) {
			return;
		}

		$featured_slider_transition_effect   = corporate_club_get_option( 'featured_slider_transition_effect' );
		$featured_slider_enable_caption      = corporate_club_get_option( 'featured_slider_enable_caption' );
		$featured_slider_enable_arrow        = corporate_club_get_option( 'featured_slider_enable_arrow' );
		$featured_slider_enable_pager        = corporate_club_get_option( 'featured_slider_enable_pager' );
		$featured_slider_enable_autoplay     = corporate_club_get_option( 'featured_slider_enable_autoplay' );
		$featured_slider_enable_overlay      = corporate_club_get_option( 'featured_slider_enable_overlay' );
		$featured_slider_transition_duration = corporate_club_get_option( 'featured_slider_transition_duration' );
		$featured_slider_transition_delay    = corporate_club_get_option( 'featured_slider_transition_delay' );
